...
allmarkers_grouped<- allmarkers %>%
    group_by(cluster) %>% slice_max(n = 1, order_by = avg_diff)
fwrite(as.data.frame(allmarkers_grouped),file="../pbmc_data/DE_markers.txt",sep="\t",row.names = F,col.names = T)

```

```{r}
allmarkers_grouped<-fread("../pbmc_data/DE_markers.txt")
# plot
features<-unique(allmarkers_grouped$gene)

StackedVlnPlot(obj = pbmc, features = features,slot = "counts", cols=my_cols)
